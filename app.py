...
def init_db():
    ...
    # Assets Table with is_deleted column
    ...
    # Ensure is_deleted exists for older DB versions
...
# --- 1. DASHBOARD & SEARCH (Filters based on Admin Role) ---
@app.route('/')
def index():
    ...
    # Logic: Admin sees deleted items, Users do not.
    ...
    # Dashboard Totals (Calculated from current view)
    stats = {
        'total': len(data),
...
def delete_asset(id):
    if 'user' not in session: return redirect(url_for('login'))
    conn = get_db_connection(); cur = conn.cursor()
    # We change is_deleted to TRUE instead of deleting from DB
    cur.execute("UPDATE assets SET is_deleted = TRUE WHERE id = %s", (id,))
    conn.commit(); cur.close(); conn.close()
    flash("Asset has been archived (Deleted).")
...
def add_asset():
    ...
    if request.method == 'POST':
        conn = get_db_connection(); cur = conn.cursor()
        t = f"JTDI-{datetime.now().strftime('%y%m%S')}"
